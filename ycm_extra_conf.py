...
def IsCXXFile( filename ):
  extension = os.path.splitext( filename )[ 1 ]
  return extension in [ '.h', '.hxx', '.hpp', '.hh', '.cc', '.cpp', '.cxx' ]

def IsKernelFile( filename ):
  topdir = os.path.dirname(os.path.abspath(filename))
  return ("/kernel" in topdir) or ("/linux-" in topdir)
...
#    return None
  ...
  if IsCXXFile(filename):
    ...
    flags.append(os.getenv("HOME") + "/src/libcxx/include")
